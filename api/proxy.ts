...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (!process.env.API_KEY) {
        return res.status(500).json({ error: 'API key is not configured on the server. Please set the API_KEY environment variable.' });
    }
    ...
    try {
        if (action === 'demystify') {
            if (!legalText) return res.status(400).json({ error: '`legalText` is required for the demystify action.' });
            ...
            if (!legalText || !question) return res.status(400).json({ error: '`legalText` and `question` are required for the answer action.' });
            ...
            return res.status(400).json({ error: 'Invalid action provided. Must be "demystify" or "answer".' });
        }
