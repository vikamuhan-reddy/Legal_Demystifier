...
const InputArea: React.FC<InputAreaProps> = ({ value, onChange, onSubmit, isLoading, isParsing, onFileSelect, selectedFile, clearInput }) => {
  ...
  const dropzoneClasses = `relative flex flex-col items-center justify-center w-full h-full p-8 text-center border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragging ? 'border-brand-secondary bg-blue-50 dark:bg-slate-700' : 'border-gray-300 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700/50'}`;
    ...
    if (mode === 'text') {
      return (
         <textarea
          ...
          className="flex-grow p-4 border border-gray-300 dark:border-slate-600 rounded-lg w-full resize-none bg-brand-light/20 dark:bg-slate-700/50 focus:ring-2 focus:ring-brand-secondary focus:outline-none transition-shadow min-h-[350px]"
        />
      );
    }
        ...
        <div className={dropzoneClasses} onClick={() => fileInputRef.current?.click()}>
            {isParsing ? (
                 <div className="flex flex-col items-center">
                    ...
                    <p className="mt-4 font-semibold text-gray-700 dark:text-gray-200">Parsing Document...</p>
                    <p className="text-sm text-gray-500">Extracting text, please wait.</p>
                </div>
            ) : selectedFile ? (
                ...
                <div className="flex flex-col items-center text-gray-500 dark:text-gray-400">
                    ...
                    <p className="font-semibold">Drag & drop a file or <span className="text-brand-secondary font-bold">click to browse</span></p>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        {hasInput && (
             <button onClick={clearInput} className="flex items-center gap-1 text-sm text-gray-500 hover:text-red-500 dark:hover:text-red-400 transition-colors">
      ...
      <button
        ...
        className="mt-4 w-full flex justify-center items-center gap-2 bg-brand-primary hover:bg-blue-800 disabled:bg-gray-400 dark:disabled:bg-slate-600 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 disabled:scale-100 disabled:cursor-not-allowed"
      >
        {isLoading ? (
          <>
